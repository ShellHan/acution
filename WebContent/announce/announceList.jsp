...
<div class="container announce">
						...
						<ul>
							<s:iterator value="#request.announces">
								<li>
										...
										<span>
										 	发布时间:<s:date name="annDate" format="yyyy-MM-dd HH:mm:ss" />
										</span>
										<font class="right" >
											<a href="ann-getAnnById?id=${id }">详情>></a>
										</font>
								</li>
							</s:iterator>		
						</ul>
	...
	<div class="pagination">
			...
			<s:if test="#request.page != 1">
				<a href="${ pageContext.request.contextPath }/ann-listAnnsTwo?pageNumer=1" class="firstPage">&nbsp;</a>
				<a href="${ pageContext.request.contextPath }/ann-listAnnsTwo?pageNumer=${page-1}" class="previousPage">&nbsp;</a>
			</s:if>
			 <s:iterator var="i" begin="1" end="#request.total">
				<s:if test="#request.page != i">
					<a href="${ pageContext.request.contextPath }/ann-listAnnsTwo?pageNumer=${i}">${i }</a>
				</s:if>
				<s:else>
			...
			<s:if test="#request.page != #request.total">	
				<a class="nextPage" href="${ pageContext.request.contextPath }/ann-listAnnsTwo?pageNumer=${page+1 }/>">&nbsp;</a>
				<a class="lastPage" href="${ pageContext.request.contextPath }/ann-listAnnsTwo?pageNumer=${total }">&nbsp;</a>
			</s:if> 
	</div>
	</form>
		</div>
